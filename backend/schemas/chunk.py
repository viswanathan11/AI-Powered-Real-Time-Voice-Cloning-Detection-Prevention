...
class ChunkAnalysisResult(BaseModel):
    ...
    verdict: Optional[str] = Field(default="SAFE_CALL", description="3-way triage classification verdict")
    verdictLabel: Optional[str] = Field(default="Natural Human Voice", description="User-friendly verdict description")
    speakerDecision: Optional[str] = Field(default="MATCH", description="Discrete verification decision: MATCH, MISMATCH, UNCERTAIN")
    audioQuality: Optional[str] = Field(default="GOOD", description="Acoustic quality assessment: GOOD, POOR_QUALITY, INSUFFICIENT_SPEECH")
    evidenceConfidence: Optional[float] = Field(default=1.0, description="Evidence confidence multiplier [0.0, 1.0]")
    classification: Optional[str] = Field(default="GENUINE", description="Triage classification: GENUINE, HUMAN_IMPERSONATOR, AI_CLONE_SUSPECTED, UNCERTAIN")
    cosineSimilarity: Optional[float] = Field(default=None, description="Raw ECAPA-TDNN cosine similarity")
